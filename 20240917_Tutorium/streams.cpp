...
void using_streams() {
	...
	/*cout << std::string("##### INFO #####\ni=")
		...
		//cin.tie(&cout);	// Bindung wird gesetzt (Standardm��ig)
...
void read_only_ints() {
	...
	while (true) {
		...
		// �berpr�fen, ob die Eingabe erfolgreich war
		if (((num > 13) && (num < 500)) || cin.fail()) {
			// Falls die Eingabe kein g�ltiger Integer war:
			cin.clear(); // Setzt den Fehlerstatus von cin zur�ck
			// Entfernt die Eingabe aus dem Eingabepuffer
			cin.ignore(numeric_limits<streamsize>::max(), '\n');
			cerr << "Ung�ltige Eingabe. Bitte eine Ganzzahl eingeben: ";
		}
		else {
			// G�ltige Eingabe
